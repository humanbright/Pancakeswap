...
export const getEarningsText = (
  ...
  const data = {
    earningsBusd: earningsBusd.toFixed(3),
    count: numFarmsToCollect,
  }

  let earningsText = t('~$%earningsBusd% ', data)

  if (numFarmsToCollect > 0 && hasCakePoolToCollect) {
    if (numFarmsToCollect > 1) {
      earningsText = t('~$%earningsBusd% to collect from %count% farms and pool', data)
    } else {
      earningsText = t('~$%earningsBusd% to collect from %count% farm and pool', data)
    }
    ...
    earningsText = t('~$%earningsBusd% to collect from pool', data)
  }
